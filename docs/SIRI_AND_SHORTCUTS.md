# Siri & Shortcuts — Miller Time

Miller Time exposes its logging and a few "ask the app" actions to Siri, Spotlight,
...
### Logging
- **"Log a feed in Miller Time"** — logs a bottle at your default amount.
  ...
  **Amount** field.)
- **"Log a diaper in Miller Time"** — logs a wet diaper.
- **"Log a dirty diaper in Miller Time"** / **"…wet diaper…"** / **"…both…"** — logs that type.
- **"Start sleep in Miller Time"** / **"Stop sleep in Miller Time"** — starts/stops the sleep timer.
- **"Undo that in Miller Time"** — removes the most recent thing you logged.

Each one speaks a confirmation back ("Logged 3 oz for Miller.").

### Asking
- **"When did Miller last eat in Miller Time"** — "Miller last ate 4 oz 1h 50m ago…"
- **"When was Miller's last diaper in Miller Time"**
- **"Is Miller asleep in Miller Time"** / **"How long has Miller been sleeping…"**
- **"How is Miller doing today in Miller Time"** — today's feeds, ounces, and diapers.
...
build a **Personal Automation** that, on a schedule, asks Miller Time *"How is
...
- **Action button** (iPhone 15 Pro and later): **Settings → Action Button →
  Controls →** pick a Miller Time control. One press logs without unlocking.
