...
**Status**: v3 — September 18, 2026 (AI features section added: on-device
Apple Intelligence plus the one Private Cloud Compute feature; v2 was July 13,
2026, v1 June 5, 2026)
...
All AI features sit behind one family-wide toggle (Settings → Feeding & Tracking → AI Features) and are off the moment it's off.

- **Predictions** (next bottle, amount, wake time) are plain statistics computed on the phone. No model, no network.
- **Daily insights and outlook** (Stats) are generated by Apple's **on-device** Foundation Models. Nothing leaves the phone; on hardware without Apple Intelligence the cards simply don't appear.
- **This week's patterns** (Stats, iOS 27+; ships switched off until Apple grants the app's Private Cloud Compute entitlement) is the **one feature whose input leaves the device**: the last two weeks of feed/sleep/diaper events (times, ounces, diaper types, and the baby's first name and age — no photos, no notes, no participant names) are sent to Apple's **Private Cloud Compute** through the Foundation Models framework. PCC runs on Apple silicon servers that process the request and discard it — Apple states the data is never stored, never accessible to Apple staff, and the software is publicly inspectable. It goes through Apple's framework, not through us: there is still no Two of Us server, no third-party service, and no API key or account of ours involved. The card's caption and the toggle's description say this in-app.
