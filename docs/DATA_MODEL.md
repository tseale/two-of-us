...
### Pause vs. revoke
`pausedAt` is a lighter, reversible sibling of `isActive`: pausing leaves the person on the `CKShare` and in the People list (so no re-invite is needed to undo it), but excludes them from the night rotation, feed/slot assignment, and "logged by" pickers, blocks their own device from logging (`EventStore.owner` / `QuickLogger`'s owner resolve to `nil`, including undo and sleep-stop, and their reminders/alarms stand down), and hides their main app UI behind `PausedAccessView`. Enforcement is **in-app only** — their CloudKit access is untouched, so widgets, the watch, and any pre-pause build on their devices can still display (and, on old builds, write) synced data. On the wire, `pausedAt = distantPast` is the explicit "not paused" (CloudKit never transmits an unset key, so a bare nil couldn't propagate a resume — same rule as `nightFirstShiftID`). Revoking (`isActive = false`) is the permanent, CKShare-backed teardown; pausing is the "not tonight" toggle for a caregiver who's between shifts.
