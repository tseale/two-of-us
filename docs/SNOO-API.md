...
- **Sessions**: `/ss/me/v10/babies/{babyId}/sessions/last` and `/ss/v2/babies/{babyId}/sessions/aggregated/?startTime=...` — the best-candidate current-host forms. **Unverified against the live service**; all paths live in `SnooAPIConfig` so a correction is a one-line change. Decoding accepts both `levels` and `detailedLevels`, both ISO-Z and bare-local timestamps.
- Manual verification step remaining (Phase 0 exit): sign in with the real account, confirm the `/ss/` paths, and capture fixtures into `TwoOfUsTests/` if the shapes differ.
