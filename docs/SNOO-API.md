...
### C.2-current Daily history — **verified-live 2026-07-31** (real account, real 200s)

- URL: `GET https://api-us-east-1-prod.happiestbaby.com/ss/me/v10/babies/{babyId}/sessions/daily`
- Query params (walked out of the endpoint's own Joi-style validation errors — `400 {"error":{"code":1001,"message":"\"timezone\" is required",...}}`):
  - `timezone` — **required**, IANA string (e.g. `America/Chicago`).
  - `startTime` — **required**. The server treats it as **wall-clock in `timezone` and ignores any zone suffix**: sending `2026-07-30T05:00:00Z` produced a window starting 05:00 *local* (first segment clipped to exactly `05:00:00.000`), not 05:00 UTC. Send the bare-local `YYYY-MM-DD HH:MM:SS.mmm` format to keep the window aligned with the user's day. Returns the 24 h window from that instant.
  - `levels=true` — include the segment array (omitting it returns only the day summary: `{"naps","longestSleep","totalSleep","daySleep","nightSleep","nightWakings"}`, durations in seconds).
  - `detailedLevels=true` — replaces `levels` with a richer array: each segment adds `device` (serial), `level` (SNOO level string), `hold`, `settings {...}`, and `sessionDetails {start, startFormatted, duration, endFormatted}`; core fields (`sessionId`, `type`, `startTime`, `stateDuration`, `isActive`) are unchanged, so either shape feeds the same decoder.
- `levels` segment shape (live capture): `{"sessionId": "2035974117684732908", "type": "asleep"|"soothing", "startTime": "2026-07-30 23:53:34.396" (bare local), "stateDuration": 14 (seconds), "isActive": false, "startTimeFormatted": "2026-07-30T23:53:34-0500"}`. Segments of one bassinet session **share a `sessionId`** and interleave asleep/soothing as the baby stirs; sessions clipped by the window boundary reappear in the adjacent day's window under the same id.
- Every `aggregated` sibling tried on the current host is a dead route (Express `Cannot GET`, authenticated): `/ss/me/v10|v11/babies/{id}/sessions/aggregated`, `.../aggregated/daily`, and bare `.../sessions`. `sessions/last` works with no query params and needs **no** Premium subscription; neither does `sessions/daily`.
...
- **Household connection**: one parent signs in; the refresh token (+ email/babyID, never the password or IdToken) is published as `SharedSettings.snooCredentials` (JSON, `SnooSharedCredentials`) in the shared CloudKit zone. Every participant's device adopts it into its own Keychain and mints its own IdTokens — safe because Cognito refresh tokens here are multi-use and un-rotated (§A.1). Sign-out anywhere clears the shared field and disconnects every device on its next sync.
...
- **Sessions**: `/ss/me/v10/babies/{babyId}/sessions/last` and `/ss/me/v10/babies/{babyId}/sessions/daily?timezone=...&startTime=...&levels=true&detailedLevels=false` — both **verified live 2026-07-31** (§C.2-current). The normaliser coalesces same-`sessionId` segments into one span and the merge step unions a session clipped across the two day windows. Decoding accepts both `levels` and `detailedLevels`, both ISO-Z and bare-local timestamps. All paths live in `SnooAPIConfig` so a correction is a one-line change.
- **Route-existence probe** (2026-07-26, **methodology corrected 2026-07-31**): an *unauthenticated* `GET` with a garbage bearer token was thought to distinguish a real route (`403 {"message":"Forbidden"}`) from a dead one (`404` Express `Cannot GET ...`). **This over-claims**: the live-account test on 2026-07-31 showed `/ss/me/v10/babies/{id}/sessions/aggregated` returns 403 to a garbage token but **404 `Cannot GET` to a valid one** — the gateway rejects bad tokens for an entire routed *prefix* before path matching, so a 403 only proves the prefix (e.g. `/ss/me/...`) reaches a service, not that the full path exists. A 404-on-garbage still reliably means dead (it 404s before auth), but "exists" claims require an authenticated request.
- **Live-account verification (2026-07-31, real sign-in on simulator)**: Cognito login → 200; `/hds/me/v11/devices` → 200 (babyID resolved from `babyIds[0]`); `/ss/me/v10/babies/{id}/sessions/last` → **200, decodes cleanly** with the current `SnooLastSessionDTO`; `/ss/me/v10/babies/{id}/sessions/aggregated` → **404 dead** (authenticated). The aggregated-day path is still unknown — see the `-snooRouteProbe` DEBUG launch arg (`TwoOfUs/SNOO/SnooRouteProbe.swift`), which sweeps candidate paths with the stored token.
